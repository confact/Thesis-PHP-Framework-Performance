...
\pagestyle {plain}
\begin{abstract}
{\bf [Context]} The new PHP Framework ideas have become popular among developers. 
It is also important to know how big an impact they have on the performance of the website. 
...
{\bf [Problem]} Visitors nowadays have less patience to wait for a website to load. Meanwhile, PHP Framework has become known among developers, but the part of the performance that reduces the load time even more, so visitors can surf without any problems as be missing. Therefore it is a good idea to try to discovery how the performance of the PHP Framework can change and improve the visitor experience.
{\bf [Contribution]} In this paper is a description of one of the first performance experiment on PHP Frameworks.
It can help people make the right decisions regarding PHP Framework in future. 
...
There are two PHP Frameworks that have the most users, one is CodeIgniter  \cite{MVC:codeigniter}, that was first released on 28 February 2006 \cite{php:codeigniter}. 
...
The Frameworks are based on MVC with add-ons, i.e. formhelpers and databasecontrollers, in various languages like PHP for example. 
...
\end{description}
My searched included more or less different types of strings like ``web development evaluation'' and ``web development performance''. 
...
There is mention of two different things in two different papers. Firstly,  \emph{Understanding Performance} contains the following comment:
\begin{quote}
...
\end{quote}
Large and many requests need a good Central Processing Unit (CPU) and network, but according to the article \emph{Understanding Performance} speed also matters but is not always the only problem. 
Secondly, in \emph{A Performance Comparison of Dynamic Web Technologies} tests are done with requests in ms:
\begin{quote}
...
{\bf Server/CPU} is important when the server needs to handle many requests at the same time. In \emph{A Performance Comparison of Dynamic Web Technologies} the following explanation can be found:
\begin{quote}
...
In \emph{A Performance Comparison of Dynamic Web Technologies} there are tests done with static content \cite{performance:dynweb} that can be images or normal HTML files. It shows it goes faster than dynamic content, but it is on request.
...
The Papers are also outdated but there have not been any new inventions in HTML performance either. 
...
\subsubsection{The Choice}
will be to create this paper as a first step in how to do a performance evaluation on PHP Framework2 because of the lack of data in this area.

\section{Experiment Design} 

\subsection{Investigated Question}
Which of the following PHP frameworks provide better performance measured in terms of {\bf page speed in ms}.
...
A good thing to know is how the server will be setup, and below is the table of the specification of the server that will be used for the experiment on the Amazon cloud.
\begin{table}[htbp]
\centering
...
Server load measured in terms of {\bf amount of request}.
...
I used the command line below to send the load request to a randomly picked ip-address\footnote[5]{The ip is typed as 11.11.11.11 to make it clear that the server is not running.} to the test server on the Amazon cloud on Ireland, both day and night.
\begin{verbatim}
...
The results you can see in Figure \ref{fig:experimentwithdb}. 
The results were almost the same on both, but a small gap between the two frameworks of just a few ms, to be exact it was a gap of 5,34 ms on a average.
\begin{figure}[h]
...
\end{figure}
\newpage
This was with database, which show if we will focus to the ms total average, are the tests run with CakePHP faster. This need to be tested without a database too, You can see the result in figure \ref{fig:experimentwithoutdb}.
\begin{figure}[h]
...
This was good to see how different the performance is between the frameworks. 
This is also a very good way to evaluate the performance on any web service or application, the load of request can show how fast the request can be handled by the framework and show the speed of it.
...
When using no database, it was static files of the page generated by the database-version of the blog, with all the posts.
The time had the difference of {\bf 1,3 ms}. 
Almost not noticeable at all.
And If we take the size in the reflection, is the gap even smaller.
...
This does not seem to be the problem, used the default tools in both framework to make a very simple blog webpage. The size should in other word be different, it would be weird if it was not. 
...
\section{Conclusion}
The frameworks performance has big difference, but mostly just if in used of database, the classes for using database can in other words be more effective. 
...
The fastest framework, according to the data is CakePHP, with just 1,3 ms in average without database and with 5,34 ms in average with database. 
This is something that is a big impact in the PHP performance and it can be fixed in newer versions. 

The experiment shows that the evaluation can be done in milliseconds for websites and web applications. 
