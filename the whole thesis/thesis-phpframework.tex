...
%\terms{Theory}
\includepdf[pages=-]{thesis_coverpage.pdf} 
...
Powerful servers can also have a impact on performance, for example a big server park in a data center. 
But, the data center cost a lot of money, which is a problem when experimenting in general.
That is why there will only be tests on cloud with one server in order to see how the framework changes the milliseconds (ms) of a request. 
...
The used strings can be seen in Table \ref{table:literaturesearchstrings}.
\begin{table}[htbp]
\centering
\begin{tabular}{ p{6cm}}
  Strings\\
...
\subsubsection{The Choice}
will be to try to make it more realistic by using server on the cloud with good internet, good hardware, and default configurations. The specification of the server is described in Table \ref{table:serverSpecification}.
\newpage
...
but the difference of the size of the page was {\bf 80,000 bytes} between the PHP frameworks. 
This could make CakePHP a winner, 
but as in my analysis my conclusion is that using default settings on the artifacts give a more realistic test environment rather then focusing on getting the same size of the pages for both frameworks.
...
This is something that has a big impact in how people will choose between the PHP frameworks, because of the PHP performance and can be fixed in future versions. 

The experiment shows that the evaluation can be done in ms for websites and web applications. 
These data show that the difference of evaluation between PHP frameworks is not a big score in ms at all, but there is a difference, even if it is a small one. 
