...
\pagestyle {plain}
\begin{abstract}
{\bf [Context]} PHP Frameworks, such as CakePHP and Codeignite, have become popular among developers since they offer ease of development save time and provide already made libraries to use. 
Considering that more and more websites are built using these frameworks it is important to know how big an impact they have on the performance of the website. 
...
% FIX! The Problem needs to have better english:
...
Which of CakePHP and Codeigniter provide better performance measured in terms of {\bf page load speed in ms}.
...
\end{figure}
\newpage
This tests with database, show if we will focus on the ms total average, the tests run faster with CakePHP. These also need to be tested without database, You can see the result in Figure \ref{fig:experimentwithoutdb}.
\begin{figure}[h]
...
The default templates in CakePHP and Codeigniter were used, making the page in different sizes, and could be a difference in the performance of the tests and requests. This was to use the defaults so the human impact would not influence the tests. It is more secure to use the defaults then trying to have the same sizes. It would be strange to try to have the same sizes because it actually is two different frameworks, making it understandable that there are difference in size.
...
This can create a gap between the frameworks, understandable with db, but without the db the tests were very close to each other. 
...
But have in mind that the size can make difference, of course.
...
but the difference of the size was 80,000 bytes. 
...
The fastest framework according to the data is CakePHP with just 1,3 ms on average without database and with 5,34 ms on average with database. 
