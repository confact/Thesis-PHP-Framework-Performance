...
This strings was used, seeing in Figure \ref{table:literaturesearchstrings}.
\begin{table}[htbp]
\centering
\begin{tabular}{ p{6cm}}
  Strings\\
...
Evaluation in web is mostly done in loads and counted in ms, but also in how to optimize the page itself to minimize request to server, like less pictures, or move them to another server. This can be seen in Figure \ref{table:rq1evaluation}.
\begin{table}[htbp]
\centering
...
There are things that can be a impact of the performance while testing. The impact i could see is listed in figure \ref{table:rq2evaluation}.
\begin{table}[htbp]
\centering
...
\begin{tabular}{ p{3cm} p{7cm} }
  	...
  	Framework & Compatible API, look at figure \ref{table:classes} for info about the classes to use.\\
...
\end{table}
I choice 5 for the concurrency level, meaning to send 5 request each time. This to help to count the response time more exactly. You can see how the tests will be done in figure \ref{table:testsforframework}.
...
\begin{description}
  ...
  \item[Environment] the server is mostly already set, see figure \ref{table:serverSpecification}.
\end{description}
...
The variables controlled was response size, amount of request and currency of requests. see the data for the variables in figure \ref{table:thedatafortests}.
\begin{table}[htbp]
\centering
