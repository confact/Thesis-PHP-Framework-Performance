...
\end{table}
\footnotetext[5]{with db it means database. In other meanings the framework with the database, the database was used with models for posts, without db it was just plain html in view with controller used.}
\footnotetext[6]{ResponseSize is total bytes of html sent from the server in one test, on every interval.}
\newpage
\subsection{The results}
The experiment was made randomly on a night in different times, picked randomly. there was 3 tests on each framework on 20,000 request total, 5 request per time when sending to the server. The results can you see in Appendix A3.
